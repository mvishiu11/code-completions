...
if st.sidebar.button("Next Example"):
    st.session_state.current_index = min(num_examples - 1, st.session_state.current_index + 1)
    st.session_state.example = dataset[st.session_state.current_index]
...
if 'label' in st.session_state.example and st.session_state.example['label'] is not None:
...
label = st.radio("Select a label:", options=[1, 0.5, 0], index=1,
                 format_func=lambda x: {1: "Correct (1)", 0.5: "Partially Correct (0.5)", 0: "Incorrect (0)"}[x])
...
for row in range(tracker_rows):
    ...
    for col_idx in range(tracker_cols):
        ...
        cols[col_idx].markdown(
            f"""
            <div style='width: 20px; height: 20px; background-color: {color}; border: 2px {border_style} black; margin: auto;'></div>
